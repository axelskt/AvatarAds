...
// proprement plutôt que de laisser la publication échouer.
function contentFor(identifier: string, caption: string): string {
  const limits: Record<string, number> = { x: 270, threads: 480 }
  const max = limits[identifier]
  if (!max || caption.length <= max) return caption
  return caption.slice(0, max - 1).replace(/\s+\S*$/, '') + '…'
}
  ...
  if (body.action === 'publish') {
    ...
    const payload = {
      ...
      posts: wanted.map((c) => ({
        integration: { id: c.id },
        value: [{
          content: contentFor(String(c.identifier || ''), caption),
    ...
    return json(200, { ok: true, published: wanted.map((c) => c.name), result })
  }
