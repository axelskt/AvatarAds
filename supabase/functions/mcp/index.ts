...
• Sinon → quality:"standard".`)
  ...
  return {
    content: [{ type: 'text', text: `[système] Génération lancée (${quality}, ${format}${kind !== 'free' ? ', ' + kind : ''}${hasRefSource ? ', produit conservé' : ''}, −${cost} cr). La carte affiche tout (progression → image). NE rappelle PAS check_image.\n${CONSIGNE_REPONSE}` }],
