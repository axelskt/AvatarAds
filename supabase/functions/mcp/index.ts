...
function toolDefs(isOwner: boolean, requireConfirm = true) {
  const tools: Array<Record<string, unknown>> = [
    ...
    {
      name: 'generate_image',
      description: `Génère une image IA (moteur AvatarAds, gpt-image) : visuel libre, STATIC AD (pub d'un produit PHYSIQUE **ou d'un SERVICE / app / SaaS**) ou photo UGC. 🧠 POUR UNE STATIC AD, C'EST TOI QUI ÉCRIS LA COPIE : d'après ce que le produit/service EST et FAIT réellement (sers-toi de ce que l'utilisateur t'a dit et du lien s'il t'en donne un), rédige un vrai titre accrocheur + 2 à 4 bénéfices VRAIS + un CTA + la marque — des arguments réels et cohérents, jamais inventés. Un format pro est ensuite choisi au hasard et gpt-image le met en forme. Pour un SERVICE, l'image de référence est une CAPTURE de l'app / un dashboard / le logo (pas un objet physique) — gpt-image rendra l'interface au lieu d'inventer un faux produit. ⛔ N'écris AUCUN texte autour de l'appel (la carte affiche le résultat) et ne consulte pas la page produit toi-même : passe son lien dans product_url. Coût : ${IMG_COST.standard} crédits en qualité standard, ${IMG_COST.high} en high. ⚠️ Qualité par défaut = TOUJOURS 'standard'. 📷 PHOTO DU PRODUIT (reproduite à l'identique), par ordre de préférence : (1) si tu as CONSULTÉ la page produit et vois l'URL de l'image principale (og:image, souvent un lien cdn.shopify.com/CDN) → passe-la dans reference_image_url (marche même si la page bloque notre serveur) ; (2) sinon l'utilisateur colle le LIEN de page → product_url (extraction auto, repli dépôt si le site est protégé) ; (3) rien de tout ça → appelle quand même l'outil, la CARTE gère (dépôt/lien/« Sans photo »). no_reference:true seulement pour un produit inventé.`,
      inputSchema: {
        type: 'object',
        properties: {
          ...
          kind: { type: 'string', enum: ['free', 'static_ad', 'ugc'], description: "'static_ad' = publicité statique d'un produit PHYSIQUE ou d'un SERVICE/app (titre + sous-titre + bénéfices + marque ; pour un physique le produit est le héros, pour un service c'est son interface/dashboard/logo) · 'ugc' = photo selfie/UGC réaliste d'une personne avec le produit · 'free' (défaut) = prompt libre." },
          reference_image_url: { type: 'string', description: "URL http(s) DIRECTE de l'image à reproduire à l'identique (…/xxx.jpg|png|webp, ≤ 10 Mo) : la PHOTO du produit physique, ou pour un SERVICE une CAPTURE de l'app / un dashboard / le logo. ASTUCE : si tu as déjà consulté la page produit et que tu vois l'URL de l'image principale (souvent un lien cdn.shopify.com ou autre CDN dans la balise og:image), passe-la ICI — elle fonctionne même quand la page bloque notre serveur." },
