...
const CONSIGNE_REPONSE = `RÉPONSE À ÉCRIRE MAINTENANT (règle STRICTE, français) : écris UNIQUEMENT « **Script vidéo proposé :** » suivi de 2 à 3 phrases (accroche + bénéfice + appel à l'action) pour une vidéo de ce produit. RIEN D'AUTRE — n'écris PAS « ton visuel arrive dans la carte », NI la description du produit, NI le récit de tes étapes (« je récupère… », « je charge l'outil… », « produit récupéré, je lance… »), NI d'options/variantes/questions.`
...
• Sinon → quality:"standard".`)
  ...
  return {
    content: [{ type: 'text', text: `[système] Génération lancée (${quality}, ${format}${kind !== 'free' ? ', ' + kind : ''}${hasRefSource ? ', produit conservé' : ''}, −${cost} cr). La carte affiche seule la progression puis l'image. NE rappelle PAS check_image, ne décris pas la carte.\n${CONSIGNE_REPONSE}` }],
