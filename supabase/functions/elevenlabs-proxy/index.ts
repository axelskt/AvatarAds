...
import { CORS, jsonRes, authUser, safeUpstream, billableGate, helperGate, applyReservationFull, settleReservation, resolveOp, releaseReservation, releaseOp } from '../_shared/guard.ts'
...
serve(async (req: Request) => {
  ...
  if (!auth.isService && auth.userId) {
    ...
    if (isBillable) { const rr = await applyReservationFull({ req, userId: auth.userId, proxy: 'elevenlabs', label: bare }); if (!rr.ok) return jsonRes(rr.status, { error: rr.error }); drawnOp = rr.opId }
  }
  ...
  try {
    ...
    if (isBillable && auth.userId) {
      if (elRes.ok) { if (drawnOp) await settleReservation(auth.userId, drawnOp) }   // livré (synchrone) → op non remboursable (fin du refund-après-TTS)
      else await releaseOp(auth.userId, drawnOp, 9999).catch(() => {})                 // amont en erreur → rendre l'op TIRÉE (resolveOp ne la retrouve pas à réserve 0)
    }
    ...
    if (isBillable && auth.userId) await releaseOp(auth.userId, drawnOp, 9999).catch(() => {})
