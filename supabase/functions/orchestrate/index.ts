...
- MOTIF D'ANIMATION (champ "motif", une valeur par scene) : c'est TOI qui choisis l'animation, d'apres CE QUI EST DIT a cet instant. Elle est utilisee par les styles visuels "page blanche" (Mot par mot, Editorial blanc) ou il n'y a ni video ni image : l'animation est alors le seul visuel, elle doit donc coller au sens.
  ...
  Laisse "" DES QUE tu hesites : un motif qui ne correspond pas exactement au mot prononce ne montre RIEN et casse la video. Rien a l'ecran vaut mieux qu'une forme decorative — le mot du sous-titre se suffit. Le vide est un choix normal et frequent.
- ANIMATION FABRIQUEE (champ "anim") — DECISIF quand aucune image ne colle : une capture d'ecran ne montre pas un CONCEPT. Le bouton "Split Screen" ne dit pas a quoi RESSEMBLE un split screen. Quand il parle d'une notion que les images fournies n'illustrent pas, DEMANDE une animation : elle est fabriquee pour toi, gratuitement, et elle montre l'idee.
  ...
  REGLE 1 — L'IMAGE PASSE TOUJOURS AVANT : si une image fournie illustre precisement ce qu'il dit a cet instant, prends l'IMAGE (broll). Une vraie capture vaut toujours mieux qu'un pictogramme. L'animation ne sert QUE quand aucune image ne colle.
  REGLE 2 — ALTERNE ET RESTE SOBRE : AU MAXIMUM 3 IMAGES et 3 animations pour une video de 30s — au-dela c'est du remplissage. Entre deux visuels, laisse la page respirer avec le mot seul.
  AU MAXIMUM 3 animations pour une video de 30s, jamais deux animations qui se suivent (intercale une image, un b-roll ou un simple plein ecran entre les deux), et jamais deux fois la MEME animation.
