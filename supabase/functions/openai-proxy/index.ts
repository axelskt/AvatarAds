...
serve(async (req: Request) => {
  ...
  // Palier 4K (x-aa-chain: nano4k, plan Élite comme dans l'app) : seulement une image gpt low/medium, n=1 → tire 5 et
  ...
  if (isBillable && gated && wantsNanoChain(req)) {
    const { plan, isOwner, err } = await userPlan(uid)
    chainPlanOk = !err && (isOwner || plan === 'elite' || plan === 'developer')
  }
