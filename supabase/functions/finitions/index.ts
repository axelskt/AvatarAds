...
4. UN MOMENT QUI MÉRITERAIT UN VISUEL ET N'EN A PAS. Les plans marqués « ton visage » sont des moments où l'on voit seulement la personne parler. C'est très bien quand elle raconte ; mais si elle NOMME une chose qu'on pourrait montrer — un chiffre, un outil, une comparaison, un résultat — il manque un visuel. Tu peux en ajouter un.

CE QUE TU NE PEUX PAS FAIRE : changer les temps, déplacer un plan, allonger ou raccourcir. Les captures d'écran (« capture … ») et les médias de l'utilisateur (« média … ») ne se remplacent ni ne se suppriment : ils ont été placés sur des règles que tu ne vois pas ici. Tu peux en revanche supprimer une animation POSÉE JUSTE À CÔTÉ d'eux si elle fait doublon avec ce qu'ils montrent déjà.
...
désigne par SON NUMÉRO (#3 → 3), jamais par son nom ni par son temps :
  ...
  supprime|7
  ajoute|5|<une animation de la liste>     ← sur un plan « ton visage » uniquement

N'AJOUTE JAMAIS POUR MEUBLER. Un visage qui parle est un plan valable, souvent le
meilleur. Tu n'ajoutes que si le mot prononcé APPELLE une image précise. Dans le
doute, tu n'ajoutes pas.

Si tout est juste, réponds exactement : RAS

Sois EXIGEANT mais pas bavard : au maximum 10 corrections, les plus flagrantes. Une correction qui remplace une animation correcte par une autre correcte ne sert à rien — tu ne corriges que ce qui est FAUX.`
...
Tes corrections :`
  ...
  try {
    ...
    for (const ligne of texte.split('\n')) {
      ...
      if (action !== 'remplace' && action !== 'supprime' && action !== 'ajoute') continue
      ...
      // les captures et les médias restent intouchables ; le VISAGE, lui, peut
      // recevoir un ajout — c'est tout l'intérêt du nouveau pouvoir.
      if (/^(capture|média|media)/i.test(cible.quoi)) { refus.push(`${t}s : ${cible.quoi} — hors de sa portée`); continue }
      if (/^ton visage/i.test(cible.quoi) && action !== 'ajoute') { refus.push(`${t}s : le visage ne se remplace pas, il se complète`); continue }
      if (action === 'supprime') { corrections.push({ action, t: cible.start, anim: '' }); continue }
      if (action === 'ajoute' && !/^ton visage/i.test(cible.quoi)) { refus.push(`${t}s : on n'ajoute que sur un plan « ton visage »`); continue }
    ...
    return json({ ok: true, corrections: corrections.slice(0, 10), refus, brut: texte.slice(0, 400) })
  } catch (e) {
