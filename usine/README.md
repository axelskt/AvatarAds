...
**Cohérence** (Axel 25/09, code : `usine/coherence.js`, même règle pour le dashboard et `publish-qc.mjs`) : toute démo peut suivre tout hook, au besoin avec une liaison **générique** (subject `generique`). `contenu.subject ∈ hook.compatible_subjects` (ou hook générique) = cohérent d'office ; sinon la vidéo part en **revue QC manuelle** (raison dans `technical.coherence`), Axel accepte ou refuse. `hook.avatar == cta.avatar`. Vidéo distincte = avatar × hook × démo (liaison, CTA, musique, sous-titres tournent).
